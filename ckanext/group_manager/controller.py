...
class GroupManager(BaseController):
    ...
    def untag(self, id, limit=50):
        ...
        try:
            ...
            abort(401, _('Unauthorized to read group %s') % id)
        self._untag(id, limit)
        return render('group_manager/untag.html')

    def _untag(self, id, limit):
        ''' This is common code used by both read and bulk_process'''
        group_type = self._get_group_type(id.split('@')[0])
        context = {'model': model, 'session': model.Session,
                   'user': c.user or c.author,
        ...
        def search_url(params):
            if group_type == 'organization':
                ...
                url = self._url_for(controller='ckanext.group_manager.controller:GroupManager', action='untag', id=id)
            params = [(k, v.encode('utf-8') if isinstance(v, basestring)
                       else str(v)) for k, v in params]
            return url + u'?' + urlencode(params)
        ...
        def drill_down_url(**by):
            return h.add_url_param(alternative_url=None,
                                   controller='ckanext.group_manager.controller:GroupManager', action='untag',
                                   extras=dict(id=c.group_dict.get('name')),
                                   new_params=by)
    ...
    def untagPackage(self, group_id, package_id):
        context = {'model': model, 'session': model.Session,
                   'user': c.user or c.author }
